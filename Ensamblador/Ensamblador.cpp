...
void writeToFile(Stack<int>* Binario)
{
  ...
  ofstream fich("salida.dat", ios::app);
  if (!fich.is_open())
  {
    cout <<"Error al abrir salida.dat\n";
    exit(EXIT_FAILURE);
  }
  fich<<hex<<endl;
  /* while(!Binario->empty())
...
// Funcion que verifica si un string esta vacio o es un comentario, el comentario se identifica por el caracter '#'
bool empty(string valor)
{
  for (unsigned int x=0; x < valor.size(); x++)
  {
    if(valor[x] == '#')
    {
      return true;
    }
    else if(valor[x] != ' '&& valor[x] != '\t' && valor[x] != '\n' && valor[x] != '\r' && valor[x] != '\v' && valor[x] != '\f')
    {
      return false;
    }
...
// Funcion que elimina identacion de un string, para facilitar su manejo
string deleteInitSpaces(string &valor)
{
  unsigned int x=0;
  while(valor[x] == ' ' || valor[x] == '\t' || valor[x] == '\n' || valor[x] == '\r' || valor[x] == '\v' || valor[x] == '\f')
  {
    x++;
  }
  valor = valor.substr(x);
  return valor;
}
...
BST<string> DataColector(const string& tipo)
{
  BST<string> arbol;
  if(tipo == "r")
  {
...
void inmediateValueCalculation(const string& codeLine, Stack<int>* Binario)
{
  string complement2;
  if (stoi(codeLine) >= 0) {
        complement2= bitset<12>(stoi(codeLine)).to_string(); // Asumiendo un tama̱o de 12 bits
    } else 
    {
        int complemento = (1 << 12) + stoi(codeLine); // Calcula el complemento a 2
        complement2= bitset<12>(complemento).to_string(); // Asumiendo un tama̱o de 12 bits
    }
  for(int x=11; x >= 0; x--)
  {
...
void getConstant(const string& codeLine, string* constant)
{
  ...
  if (stoi(stringVersion) >= 0) {
        stringVersion= bitset<12>(stoi(stringVersion)).to_string(); // Asumiendo un tama̱o de 12 bits
    } else 
    {
        int complemento = (1 << 12) + stoi(stringVersion); // Calcula el complemento a 2
        stringVersion= bitset<12>(complemento).to_string(); // Asumiendo un tama̱o de 12 bits
    }
...
Stack<int>* JtypeGeneration(const Vector<string>& codeline, int& offset)
{
  ...
  if (offset >= 0) 
  {
    complement2= bitset<21>(offset).to_string(); // Asumiendo un tama̱o de 20 bits
  } 
  else 
  {
    int complemento = (1 << 21) + offset; // Calcula el complemento a 2
    complement2= bitset<21>(complemento).to_string(); // Asumiendo un tama̱o de 20 bits
  }
  for(int x=8; x >= 1; x--)
  {
    Binario->push(complement2[x]-'0');
...
Stack<int>* BtypeGeneration(const Vector<string>& codeline,int& offset)
{
  ...
  string complement2;
  if (offset >= 0) {
      complement2= bitset<13>(offset).to_string(); // Asumiendo un tama̱o de 12 bits y que vamos a descartar el primero
  } else 
  {
...
int main()
{
  // Abre el fichero In.dat en modo lectura
  ifstream fich("entrada.dat", ios::in);
  if (!fich.is_open())
  {
    cout <<"Error al abrir entrada.dat\n";
    exit(EXIT_FAILURE);
  }
  ...
  Rtype = DataColector("r");
  Itype = DataColector("i");
  Jtype = DataColector("j");
  Stype = DataColector("s");
  Utype = DataColector("u");
  Btype = DataColector("b");
  pseudoInstructions = DataColector("p");
